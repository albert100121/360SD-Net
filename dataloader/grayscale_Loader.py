...
def default_loader(path):
    return np.tile( np.expand_dims(np.array(Image.open(path).convert('LA'))[:,:,0], 2), (1,1,3))    # convert RGB image to grayscale and copy for 3 channels
...
class myImageFolder(data.Dataset):
    def __init__(self, equi_infos, up, down, up_disparity, training, loader=default_loader, dploader= disparity_loader):
