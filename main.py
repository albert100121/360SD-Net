...
parser.add_argument('--start_learn', type=int, default=50,
...
# Load Data ------------------------------------------------------------------------------
train_up_img, train_down_img, train_up_disp, valid_up_img, valid_down_img, valid_up_disp = lt.dataloader(args.datapath, args.datapath_val)
Equi_infos = equi_info
TrainImgLoader = torch.utils.data.DataLoader(
         DA.myImageFolder(Equi_infos, train_up_img,train_down_img,train_up_disp, True), 
         batch_size= args.batch, shuffle= True, num_workers= 8, drop_last=False)

ValidImgLoader = torch.utils.data.DataLoader(
         DA.myImageFolder(Equi_infos, valid_up_img,valid_down_img,valid_up_disp, False), 
         batch_size= args.batch, shuffle= False, num_workers= 4, drop_last=False)
#-----------------------------------------------------------------------------------------
...
def main():
    ...
    for epoch in tqdm(range(start_epoch+1, args.epochs+1), desc='Epoch'):
        ...
        # Train --------------------------------------------------------------------------------------------------
        for batch_idx, (imgU_crop, imgD_crop, disp_crop) in tqdm(enumerate(TrainImgLoader), desc='Train iter'):
            loss = train(imgU_crop,imgD_crop, disp_crop)
            total_train_loss += loss
            global_step+=1
            writer.add_scalar('loss',loss,global_step) # tensorboardX for iter
        writer.add_scalar('total train loss',total_train_loss/len(TrainImgLoader),epoch) # tensorboardX for epoch
        #---------------------------------------------------------------------------------------------------------
        ...
        for batch_idx, (imgU, imgD, disp) in tqdm(enumerate(ValidImgLoader), desc='Valid iter'):
            ...
            global_val+=1
            #------------
        writer.add_scalar('total validation loss',total_val_loss/(len(ValidImgLoader)),epoch) #tensorboardX for validation in epoch        
        writer.add_scalar('total validation crop 26 depth rmse',total_val_crop_rmse/(len(ValidImgLoader)),epoch) #tensorboardX rmse for validation in epoch
    writer.close()
    # End Training
